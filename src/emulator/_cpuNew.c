 ...
 * @file _cpuNew.c
 *
 * MIPS instruction emulation for Wii. This file is included into cpu.c
...
static bool fn_8000E734(Cpu* pCPU, s32 nOpcode, s32 nOpcodePrev, s32 nOpcodeNext, s32 nAddress) {
        ...
        if (!bInterpret) {
            ...
            switch (MIPS_OP(nOpcode)) {
                ...
                case 0x23: // lw
                    ...
                    //         pCPU->nFlagCODE |= 2;
                    //     }
                    // } else if (gpSystem->eTypeROM == CLBJ || gpSystem->eTypeROM == CLBE || gpSystem->eTypeROM == CLBP) {
                    //     if (nOpcode == 0x8C9F0004 && nOpcodePrev == 0x8C9D0000 && nOpcodeNext == 0x8C900008) {
                    //         pCPU->nFlagCODE |= 2;
                    //     }
                    // } else if (gpSystem->eTypeROM == NFXJ || gpSystem->eTypeROM == NFXE || gpSystem->eTypeROM == NFXP) {
                    ...
                    //     }
                    // }
                    fn_8000E734(pCPU, nOpcode, nOpcodePrev, nOpcodeNext, nAddress);
                    if (pCPU->nFlagRAM & (1 << MIPS_RS(nOpcode))) {
                        if ((iRegisterA = ganMapGPR[MIPS_RT(nOpcode)]) & 0x100) {
                            iRegisterA = 5;
                        }
...
#include "emulator/_cpuDecodePPC3.c"
