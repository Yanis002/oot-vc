...
//! TODO: move once frame.c is matched?
extern f32 lbl_8025D090;
extern f32 lbl_8025D094;
...
static TevColorOp sUsualOps[] = {
    {GX_TEV_ADD, GX_TB_ZERO, GX_CS_SCALE_1, 0, GX_TEVPREV}, {GX_TEV_SUB, GX_TB_ZERO, GX_CS_SCALE_1, 0, GX_TEVPREV},
    {GX_TEV_SUB, GX_TB_ZERO, GX_CS_SCALE_1, 1, GX_TEVPREV}, {GX_TEV_ADD, GX_TB_ZERO, GX_CS_SCALE_1, 1, GX_TEVPREV},
    {GX_TEV_ADD, GX_TB_ZERO, GX_CS_SCALE_1, 1, GX_TEVREG0},
};
...
static inline void BuildTevMemset(void* ptr, int value, size_t num) {
    u8* p = ptr;
...
CombineModeTev* BuildCombineModeTev(u32 color1, u32 alpha1, u32 color2, u32 alpha2, u32 numCycles, u32 arg5, u32 arg6) {
    static u32 image[16] ATTRIBUTE_ALIGN(32) = {0x0024486C, 0x90B4D8FF};
    ...
    if (arg6 == 0) {
        for (i = 0; i < numCycles; i++) {
            for (j = 0; j < 4; j++) {
                if (stageValues[i][0][j] == 1 || stageValues[i][0][j] == 2) {
                    stageValues[i][0][j] = 6;
                }

                if (stageValues[i][1][j] == 1 || stageValues[i][1][j] == 2) {
                    stageValues[i][1][j] = 6;
                }
            }
        }
    ...
    for (i = 0; i < 4; i++) {
        if (stageValues[0][0][i] == 13 || stageValues[0][1][i] == 0) {
            break;
        }
    }
    ...
    if (i < 4) {
        if (arg5 != 0) {
            bVar2 = true;
            tevStages.tevOrder[0].coordID = GX_TEXCOORD2;
            tevStages.tevOrder[0].mapID = GX_TEXMAP2;
            tevStages.tevOrder[0].chanID = GX_COLOR_NULL;
            tevStages.tevColorOpP[0][1] = tevStages.tevColorOpP[0][0] = sUsualOps[4];
            tevStages.tevColorArg[0][3] = GX_CC_TEXA;
            tevStages.tevAlphaArg[0][3] = GX_CA_TEXA;
            tevStages.numStages = tevStages.numStages++;

            GXInitTexObj(&sTexObj, (void*)image, 8, 8, GX_TF_I8, GX_CLAMP, GX_CLAMP, 0);
            GXLoadTexObj(&sTexObj, GX_TEXMAP2);
            thing = (1.0f / ((lbl_8025D090 - lbl_8025D094) * 0.2857143f));
            memset(matrix, 0, sizeof(Mtx));
            matrix[0][3] = thing * -3680.0f;
            matrix[0][2] = thing * -1.0f;
            GXLoadTexMtxImm(matrix, 0x24, GX_MTX2x4);
            GXSetTexCoordGen2(GX_TEXCOORD2, GX_TG_MTX2x4, GX_TG_POS, 0x24, 0, 0x7D);
        } else {
            tevStages.tevOrder[0].coordID = GX_TEXCOORD_NULL;
            tevStages.tevOrder[0].mapID = GX_TEXMAP_NULL;
            tevStages.tevOrder[0].chanID = GX_COLOR_NULL;
            tevStages.tevColorOpP[0][1] = tevStages.tevColorOpP[0][0] = sUsualOps[4];
            tevStages.tevColorArg[0][3] = GX_CC_ONE;
            tevStages.tevAlphaArg[0][3] = GX_CA_KONST;
            tevStages.numStages++;
        }
    }
    ...
    if ((tevStages.numTexGen & 2)) {
        tevStages.numTexGen = 2;
    } else {
        if (tevStages.numTexGen != 0) {
            tevStages.numTexGen = 1;
        }
    }

    if (bVar2 == true) {
        tevStages.numTexGen = 3;
    }
